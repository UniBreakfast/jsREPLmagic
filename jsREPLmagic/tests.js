...
((tests) => {
  ...
  tFuncResult = (func, args, result, arg_str, res_str) => {
    ...
    log(`Test ${Tnum++}:  ${func.name}(${arg_str || rdbl(args, 1)}) = ${res_str || rdbl(result)}    ` +
  ...
  tFuncProbe = (func, args, probe_func, arg_str) => {
    args = Array.isArray(args) ? args : [args]
    log(`Test ${Tnum++}:  ${func.name}(${arg_str || rdbl(args, 1)});  ${probe_func.name}()    ` + (probe_func(func.apply(this, args) || args) ?
        'YES!' : 'NO...'))
  }
  tFuncAssert = (func, args, assert_code, arg_str, asc_str) => {
    let arg = args = Array.isArray(args) ? args : [args]
    let result = func.apply(this, args)
    log(`Test ${Tnum++}:  ${func.name}(${arg_str || rdbl(args, 1)});  ${asc_str || assert_code}    ` + (eval(assert_code) ? 'YES!' : 'NO...'))
  }

  tMethResult = (obj, meth, args, result, obj_str, arg_str, res_str) => {
    if (!obj[meth]) return log(`no ${meth} method on ${obj_str} found!`)
    args = Array.isArray(args) ? args : [args]
    log(`Test ${Tnum++}:  ${obj_str}.${meth}(${arg_str || rdbl(args, 1)}) = ${res_str || rdbl(result)}    ` + (obj[meth].apply(obj, args) == result ||
  ...
  tMethProbe = (obj, meth, args, probe_func, obj_str, arg_str) => {
    if (!obj[meth]) return log(`no ${meth} method on ${obj_str} found!`)
    args = Array.isArray(args) ? args : [args]
    log(`Test ${Tnum++}:  ${obj_str}.${meth}(${arg_str || rdbl(args, 1)});  ${probe_func.name}()    ` + (probe_func(obj, args, obj[meth].apply(obj, args) ||
  ...
  tFuncProbe(neld, 'myDiv', function div_w_id_created(res) { return res.tagName == 'DIV' && res.id == 'myDiv' })
  //Test: neldr('myDiv') returns <div> with rounded corners
  tFuncProbe(neldr, _, function div_w_rounded_corners(res) { return res.s.borderRadius == '15%' })

  //Test: evo([<div>, <div>]) -> works on multiple elements at once
  tFuncProbe(evo, [neld(), neld()],
    function multiple_elements_at_once(res) { return res[0].s == res[0].style && res[1].s == res[1].style },
    "[<div>,<div>]")
  log('')

  //Test: el1.a(el2) -> appends el2 to el1
  tMethProbe(neld('out'), 'a', neld('in'),
    function appended_a_child(obj) { return obj.children[0].id == 'in' },
    '<div id=out>', '<div id=in>')
  //Test: el1.a2(el2) -> appends el1 to el2
  tMethProbe(neld('in'), 'a2', neld('out'),
    function appended_as_a_child(obj, args) { return obj.parentElement == args[0] },
    '<div id=in>', '<div id=out>')

  //Test: el.tx('text') -> sets innerText of an element
  tMethProbe(neld(), 'tx', 'text',
    function sets_innerText_of_an_element(obj, args) { return obj.innerText == args[0] },
    '<div>')
  //Test: el.htm('<p>text</p>') -> sets innerHTML of an element
  tMethProbe(neld(), 'htm', '<p>text</p>',
    function sets_innerHTML_of_an_element(obj, args) { return obj.innerHTML == args[0] },
    '<div>')
  ...
  tMethProbe(neld(), 'sc', [20, 10, 3, 'teal'],
    function sets_size_and_colors(obj, args) { return obj.s.c() == 'teal' }, "<div>")
